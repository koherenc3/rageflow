...
The one bound on it is today. The window the engine publishes and classifies against runs to the end she typed or to today, whichever comes first, and the days between take their ordinary phase until they arrive. Read cold that looks like the previous paragraph reversed, and it is not: showing a 22 day bleed that already happened is showing her data as it is, while asserting that she is bleeding on days that have not happened is not showing data at all, it is a claim about the future, and a logged fact about the future is not a fact. Length and time are two different questions and only the second one is bounded here. The entry itself is untouched in both cases: nothing is rewritten, and the day it names becomes a period day the moment it comes round.
...
The paragraph that used to sit here recorded the opposite reasoning: that a bleed drawn weeks out would strip `fertileWindow`, `estimatedOvulationDate` and `premenstrualWindow` from the model, indistinguishable from the suppression that means something. That was the wrong way round. It is the suppression that means something, and it means the thing this whole section is about, which is that her entry outranks the estimate. A fertility estimate published across days she wrote down as a period is the failure, not the cure for one.
