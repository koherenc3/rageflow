...
There is one span, so both jobs get the bound together. That is deliberate and it is the part with a cost: past the bound the days of an over-long entry that have not arrived no longer hold the fertile window back, so a fertility estimate can be published over days her entry names and then disappear as those days arrive and become facts. That is accepted, narrowly. Any day it can happen on is past the plausible bleed bound, which is to say inside an entry the fit has already judged not to be a period, and it has not happened yet. Suppressing fertility for as long as a typo happens to run is the larger failure of the two, because the suppression is indistinguishable from the one that means something.
