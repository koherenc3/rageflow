...
Both states are read off `today` and never off the date being asked about, so a calendar querying next Tuesday gets next Tuesday's ordinary phase, and no date before the predicted start is touched by either. From the predicted start onward both states do reach past today, deliberately. The predicted bleed is the only thing `phaseForDate` claims past that date, since a cycle whose start has not happened cannot place anything after it, and once today is `late` those days report `late` as well: the engine predicted a period, watched nothing arrive, and must not then paint five days of a month calendar as a period its own report on today says did not happen. Once the log is stale the predicted windows are withheld for every date in the in-progress cycle rather than only for today. Completed cycles are never `late` or `stale`, since they are bounded by a real next start.
