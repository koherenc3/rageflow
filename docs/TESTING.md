...
| `phases.test.ts`      | The normal-normal learned lengths, the luteal update hook, the phase of every day of a worked 28 day cycle, and the `late` and `stale` states either side of their boundary, including that both key off today rather than the date being asked about, and that a bleed the engine predicted and never saw arrive is not painted onto the calendar |
